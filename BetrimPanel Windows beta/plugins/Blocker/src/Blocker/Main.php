			...
			case "ban":
			   ...
			   if(!isset($argument[0], $argument[1])){
			   	   ...
			   	   if($sender->getServer()->getPlayer($argument[0])){
					   ...
					   $player = $sender->getServer()->getPlayer($argument[0]);
			   	   	   $sender->getServer()->getNameBans()->addBan($player->getName(), $argument[1], null, $sender->getName());
			   	   	   $player->close("§cВаш аккаунт заблокирован", false);
